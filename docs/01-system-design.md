...
「日本神秘紀行 ～神々の記憶を巡る旅～」は、ユーザーが日本地図から神秘スポットを巡り、スポットを解放しながら神話・歴史・伝説・絶景・BGM・AIガイドを楽しむWebアプリです。
...
ユーザーがスポット詳細から「この絶景を解放する」を押すと、`collections` にレコードを作成します。

すでに解放済みの場合は重複登録せず、解放済み状態を返します。

### 神秘ポイント

初回解放時に、スポットの `mystic_points` をユーザー体験上の獲得ポイントとして扱います。
